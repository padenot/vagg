...
#define VAGG_SYSCALL(x)                                                        \
        ...
        if(x == -1) {                                                          \
          fprintf(stderr, "%s#! SYSCALL FAILED : %s:%d:%s:%s%s\n",             \
                  VAGG_COLORS[VAGG_LOG_FATAL], __FILE__, __LINE__,             \
                  #x, strerror(errno), VAGG_COLORS[VAGG_COLOR_RESET]);         \
          abort();                                                             \
        }                                                                      \
        VAGG_MACRO_END
  ...
  #define VAGG_LOG_UNINDENT VAGG_ASSERT(vagg_log_indent_level-- > (size_t)0,   \
 ...
 */
  ...
  #define vagg_set_exit_handler()                                              \
    if(atexit(trace_exit_handler)) {                                           \
      VAGG_LOG(VAGG_LOG_FATAL, "Couldn't set exit function.\n");               \
      abort();                                                                 \
    }
